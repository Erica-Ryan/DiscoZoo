...
```{python}
#First I'm going to make a list that takes in the animal list and makes a new list
#first thing is the animal, second thing is all possible locations of that animal
...
#next I'm going to make a list of all the combinations of the patterns in allpat
#I'm not really going to care how many of them are actually feasible for the moment
...
#Now I'm going to care about possible options. If more than two animal keys are in the same square, it is removed.
...
```{python}
#Now I want to build a master list that gives me the number of possible options.
def numopts(possibleopts):
    for r in range(len(possibleopts)):
        if r == 0 :
            newopts = possibleopts[r]
        else:
            newopts = np.core.defchararray.add(newopts, possibleopts[r])
        
    newoptscount = np.empty((dim,dim))
    for s in range(dim):
        for t in range(dim):
            newoptscount[s][t]=len(newopts[s][t])/2
    return(newoptscount)


def percents(possibleopts, newoptscount):
    x = len(possibleopts)
    half = newoptscount * (1/x)
    return(half) 
```

```{python}
#need to rethink this one. 
#UPDATE. Rethought it.  Jazzy.

def animalspecific(options, anlist):
    output = []
    for animal in range(len(anlist)):
        newoptscount = np.zeros((dim,dim))
        an = Animals(anlist[animal]).abbr
    
        for option in range(len(options)):
            for rowz in range(dim):
                for colz in range(dim):
                    if options[option][rowz][colz] == an:
                        newoptscount[rowz][colz] = newoptscount[rowz][colz]+1
        
        output.append([Animals(anlist[animal]).name, newoptscount])
    return(output)
```

```{python}
originalpossible = numopts(ogpossible)
percentages = percents(ogpossible, originalpossible)
test = animalspecific(ogpossible, AnimalList)
test
```

```{python}
#This was a first attempt and I maybe spent half an hour thinking about it and realized it wouldn't really have all that much value but I feel bad just deleting it. Thank goodness for Dev code. It'll disappear before prod.

#def animalspecificDEAD(everypattern):
#    animalspecific = []    
#    
#    for an in range(len(everypattern)):
#        for item in range(len(everypattern[an][1])):
#            if item == 0:
#                temparray = everypattern[an][1][item]
#            else:
#                temparray = np.core.defchararray.add(temparray, everypattern[an][1][item])
#                
#        tempcount = np.empty((dim,dim))
#        for rows in range(dim):
#            for cols in range(dim):
#                tempcount[rows][cols]=len(temparray[rows][cols])/2
#                    
#        animalspecific.append([an, temparray, tempcount])
#    return(animalspecific)
#
#animalspecificpattern = animalspecific(allpat)
```
...
2/17/20 - More Overhaul and man did I not bother to look at my notes from my previous overhaul! Good programming practice right? I made a neat function called animal specific that I'm hoping will be useful for understanding the true value of the squares. I now have a function that will generate the total number of options per square, and the total percentages. Oh man I should make my percentages function able to take in a list that would really make that dynamic. Gonna do that next time if I REMEMBER TO READ MY NOTES CMON 
